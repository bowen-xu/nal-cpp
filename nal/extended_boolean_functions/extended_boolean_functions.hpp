#pragma once
#include "extended_boolean_functions.decl.hpp"
#include <vector>
...
inline double And(std::initializer_list<double> x)
{
    return std::accumulate(x.begin(), x.end(), 1.0, [](double acc, double xi) { return acc * xi; });
}

inline double And(std::vector<double> x)
{
...
inline double Or(std::vector<double> x)
{
    return 1.0 - std::accumulate(x.begin(), x.end(), 1.0, [](double acc, double xi) { return acc * (1.0 - xi); });
}
